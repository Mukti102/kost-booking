<!-- Testimonial Section -->
<section class="py-20 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16">
            <h2 class="text-4xl font-bold text-gray-900 mb-4">Apa Kata Penghuni Kami</h2>
            <p class="text-gray-600 text-lg">Testimoni dari penghuni yang puas</p>
        </div>

        <div class="grid md:grid-cols-3 gap-8 overflow-x-auto snap-x snap-mandatory md:overflow-visible md:snap-none scroll-smooth">

            @forelse ($testimonis as $item)
                <div class="bg-white p-6 rounded-xl shadow-lg snap-center min-w-[85%] sm:min-w-[45%] md:min-w-0">
                    
                    {{-- Profile --}}
                    <div class="flex items-center mb-4">
                        <img src="{{ $item->photo 
                            ? asset('storage/'.$item->photo) 
                            : 'https://ui-avatars.com/api/?name='.urlencode($item->name).'&background=4F46E5&color=fff' }}"
                            alt="{{ $item->name }}" 
                            class="w-12 h-12 rounded-full mr-4">

                        <div>
                            <h4 class="font-semibold text-gray-900">{{ $item->name }}</h4>
                            <p class="text-sm text-gray-500">{{ $item->jabatan }}</p>
                        </div>
                    </div>

                    {{-- Rating --}}
                    <div class="flex mb-3">
                        @for ($i = 1; $i <= 5; $i++)
                            <svg class="w-5 h-5 {{ $i <= $item->rate ? 'text-yellow-400' : 'text-gray-300' }}"
                                 fill="currentColor" viewBox="0 0 20 20">
                                <path
                                    d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z">
                                </path>
                            </svg>
                        @endfor
                    </div>

                    {{-- Comment --}}
                    <p class="text-gray-600 italic">
                        "{{ $item->comment }}"
                    </p>
                </div>
            @empty
                <div class="col-span-3 text-center text-gray-500">
                    Belum ada testimoni.
                </div>
            @endforelse

        </div>
    </div>
</section>
